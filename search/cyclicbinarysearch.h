...
class CyclicBinarySearch {
    ...
    private:
        int cyclic_binary_search(vector<T> v, int l, int r, int value) const;
};

template <typename T>
int CyclicBinarySearch<T>::operator()(vector<T> & v, int value) const {
    return cyclic_binary_search(v, 0, v.size()-1, value);
}

// O(lg(N)) time complexity and O(1) space complexity
template <typename T>
int CyclicBinarySearch<T>::cyclic_binary_search(vector<T> v, int l, int r, int value) const {
    if(l > r) return -1;
